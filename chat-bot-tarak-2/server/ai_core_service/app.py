...
try:
    # Added podcast_service to the imports
    from . import config, file_parser, faiss_handler, llm_handler, llm_router, agent, podcast_service
    ...
    # Added podcast_service to the imports
    import config, file_parser, faiss_handler, llm_handler, llm_router, agent, podcast_service
    from tools import web_search as web_search_module
    from tools.document_search_tool import smart_search
    from tools.web_search_tool import web_search
    from tools.quiz_generator_tool import quiz_generator_tool
...
def agent_invoke_route():
    ...
    if not all([user_id, query, api_keys]):
        return create_error_response("Missing required fields: user_id, query, api_keys", 400)

    try:
        router = llm_router.LLMRouter(api_keys=api_keys)
        reasoning_handler = router.get_llm_for_task(task_type="reasoning")
        agent_llm = reasoning_handler.client
