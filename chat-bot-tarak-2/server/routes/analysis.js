...
router.post('/document', tempAuth, async (req, res) => {
    ...
    try {
        ...
        console.log('[DEBUG] Python AI service response data:', pythonResponse.data);
        if (pythonResponse.data?.analysis_result) {
            console.log('[DEBUG] Analysis Result:', pythonResponse.data.analysis_result);
        }
