...
router.post('/agentic', tempAuth, async (req, res) => {
    ...
    try {
        // 1. Get user's API keys. We need this to pass to the agent.
        // We can just use "gemini" as a placeholder provider since getApiAuthDetails needs one.
        const { apiKeys } = await getApiAuthDetails(userId, "gemini");
        ...
        const pythonResponse = await axios.post(`${PYTHON_AI_SERVICE_URL}/agent/invoke`, pythonPayload, { 
            timeout: 300000 // Agentic tasks can take a long time, so use a long timeout (5 minutes)
        });
        ...
        const agentResponseMessage = {
            ...
            timestamp: new Date(),
            isAgent: true,
            // ADD THIS NEW FIELD, getting the data from the Python response
...
router.post('/refine-prompt', tempAuth, async (req, res) => {
    ...
    try {
        // Get user's API keys to pass to the service
        // Groq is used by default, so we check for its key.
        const { apiKeys } = await getApiAuthDetails(userId, "groq");
        ...
        // Call the new /chat/refine-prompt endpoint in the Python service
